...
Installed binaries can still run `ebpf-tracker demo` and `ebpf-tracker see`
because the CLI bundles demo assets and materializes them into cache on demand.
This directory remains the source of truth for clone-local editing and `cargo`
aliases.
